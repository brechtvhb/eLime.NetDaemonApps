...
public class FlexiScreen : IDisposable
{
    ...
    private async Task UpdateStateInHomeAssistant()
    {
        ...
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_automated_state_change", LastAutomatedStateChange?.ToString("O") ?? "None");
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_manual_state_change", LastManualStateChange?.ToString("O") ?? "None");
