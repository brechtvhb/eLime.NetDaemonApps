...
public class FlexiScreen : IDisposable
{
    ...
    private async Task EnsureSensorsExist()
    {
        ...
        _logger.LogDebug("{Screen}: Upserting entities in home assistant.", Name);
    ...
    private async Task UpdateStateInHomeAssistant()
    {
        ...
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_state_change_triggered_by", LastStateChangeTriggeredBy != null ? LastStateChangeTriggeredBy.Value.ToString() : "None");
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_automated_state_change", LastAutomatedStateChange != null ? LastAutomatedStateChange.Value.ToString("O") : "None");
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_manual_state_change", LastManualStateChange != null ? LastManualStateChange.Value.ToString("O") : "None");
