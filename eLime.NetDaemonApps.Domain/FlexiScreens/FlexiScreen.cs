...
public class FlexiScreen : IDisposable
{
    ...
    private async Task UpdateStateInHomeAssistant()
    {
        ...
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_state_change_triggered_by", LastStateChangeTriggeredBy?.ToString() ?? "default");
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_automated_state_change", LastAutomatedStateChange?.ToString("O") ?? "default");
        await _mqttEntityManager.SetStateAsync($"{baseName}_last_manual_state_change", LastManualStateChange?.ToString("O") ?? "default");
