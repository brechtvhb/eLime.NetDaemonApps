...
public class CarChargerEnergyConsumer : EnergyConsumer, IDynamicLoadConsumer
{
    ...
    {
        var needsEnergy = (StateSensor.State == CarChargerStates.Occupied.ToString() || StateSensor.State == CarChargerStates.Charging.ToString()) && Cars.Any(x => x.CableConnectedSensor.IsOn() && x.BatteryPercentageSensor.State < 100);
