...
public class GridMonitor : IDisposable
{
    ...
    {
        return _lastImportValues.Where(x => x.Moment.Add(timeSpan) > now).Select(x => x.Value).Average();
    }

    public double AverageExportSince(DateTimeOffset now, TimeSpan timeSpan)
    {
        return _lastExportValues.Where(x => x.Moment.Add(timeSpan) > now).Select(x => x.Value).Average();
    }
