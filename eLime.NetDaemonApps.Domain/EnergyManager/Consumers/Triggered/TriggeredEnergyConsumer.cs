...
public class TriggeredEnergyConsumer : EnergyConsumer
{
    ...
    internal TriggeredEnergyConsumer(EnergyManagerContext context, EnergyConsumerConfiguration config)
        : base(context, config)
    {
        if (config.Triggered == null)
            throw new ArgumentException("Simple configuration is required for TriggeredEnergyConsumer.");
