...
public class CoolingEnergyConsumer : EnergyConsumer
{
    ...
    internal CoolingEnergyConsumer(EnergyManagerContext context, EnergyConsumerConfiguration config)
        : base(context, config)
    {
        if (config.Cooling == null)
            throw new ArgumentException("Cooling configuration is required for CoolingEnergyConsumer.");
