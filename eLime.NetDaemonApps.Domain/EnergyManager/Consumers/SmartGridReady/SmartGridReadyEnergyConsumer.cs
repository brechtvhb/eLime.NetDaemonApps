...
public class SmartGridReadyEnergyConsumer : EnergyConsumer, IDynamicLoadConsumer
{
    ...
    private void PowerConsumptionSensor_Changed(object? sender, Entities.NumericSensors.NumericSensorEventArgs e)
    {
        try
        {
            ...
            switch (State.State)
            {
                case EnergyConsumerState.Running when e.Sensor.State > 100:
                    Started();
                    break;
                case not EnergyConsumerState.Running when e.Sensor.State <= 100 && SmartGridReadyMode != SmartGridReadyMode.Boosted:
                    Stopped();
                    break;
            }
        }
