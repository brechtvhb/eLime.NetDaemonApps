...
public class SmartWasherBuilder
{
    ...
    public SmartWasherBuilder(AppTestContext testCtx, ILogger logger, IMqttEntityManager mqttEntityManager, IScheduler scheduler)
    {
        _testCtx = testCtx;
        _logger = logger;
    ...
    public SmartWasher Build()
    {
        ...
        var smartWasher = new SmartWasher(_logger, _testCtx.HaContext, _mqttEntityManager, _scheduler, _config.Enabled ?? true, _config.Name, powerSocket, powerSensor);
        return smartWasher;
    }
}