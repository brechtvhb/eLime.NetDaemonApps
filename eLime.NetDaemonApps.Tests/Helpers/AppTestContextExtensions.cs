...
public static class AppTestContextExtensions
{
    ...
    public static void InputNumberChanged(this AppTestContext ctx, string entity, double value, Func<Times> times)
    {
        ctx.HaContextMock.Verify(c => c.CallService("input_number", "set_value", It.Is<ServiceTarget>(s => Match(entity, s)), It.Is<InputNumberSetValueParameters>(x => x.Value == value)), times);
    }
    ...
    public static void NumberChanged(this AppTestContext ctx, string entity, double value, Func<Times> times)
    {
        ctx.HaContextMock.Verify(c => c.CallService("number", "set_value", It.Is<ServiceTarget>(s => Match(entity, s)), It.Is<InputNumberSetValueParameters>(x => x.Value == value)), times);
    }
