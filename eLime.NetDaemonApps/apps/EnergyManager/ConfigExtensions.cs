        ...
        foreach (var consumer in config.Consumers)
        {
            ...
            if (consumer.CarCharger != null)
            {
                ...
                foreach (var car in consumer.CarCharger.Cars)
                {
                    ...
                    cars.Add(new Car(car.Name, mode, carCurrentEntity, car.MinimumCurrent, car.MaximumCurrent, car.BatteryCapacity, batteryPercentageSensor, maxBatteryPercentageSensor, cableConnectedSensor, location, scheduler));
                }
